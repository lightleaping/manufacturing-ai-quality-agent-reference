...
def explain_failure_prediction(
    ...
    """
    확정된 Failure Prediction 결과를
    OpenAI 기반 쉬운 운영 해설로 변환합니다.

    이 Endpoint는 Prediction을 다시 수행하지 않습니다.

    OpenAI 호출에 실패해도 HTTP 오류로
    기존 Prediction 결과를 무효화하지 않습니다.

    추가 설명 실패는 Response의 error 필드로
    예측 결과와 분리하여 반환합니다.
    """
